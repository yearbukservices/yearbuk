...
export default function PhotosMemoriesManage() {
  ...
  const showNextPendingMemoryOrClose = (memoryId: string) => {
    const currentIndex = pendingMemories.findIndex((memory: any) => memory.id === memoryId);
    const nextMemory = currentIndex >= 0 ? pendingMemories[currentIndex + 1] : undefined;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-950 relative overflow-hidden">
      ...
      {showHamburgerMenu && (
        <div className="hamburger-dropdown fixed top-16 right-2 sm:right-4 w-48 bg-blue-950/95 backdrop-blur-lg border border-white/20 rounded-lg shadow-xl z-[999999]">
          <div className="py-1">
            <button
                ...
                {previewMemory.status === 'pending' ? (
                  ...
                  <>
                    <Button
                      ...
                      onClick={() => {
                        const memoryId = previewMemory.id;
                        showNextPendingMemoryOrClose(memoryId);
                        approveMemoryMutation.mutate(memoryId);
                      }}
                      disabled={approveMemoryMutation.isPending || denyMemoryMutation.isPending}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const memoryId = previewMemory.id;
                        showNextPendingMemoryOrClose(memoryId);
                        denyMemoryMutation.mutate(memoryId);
                      }}
                      disabled={approveMemoryMutation.isPending || denyMemoryMutation.isPending}
